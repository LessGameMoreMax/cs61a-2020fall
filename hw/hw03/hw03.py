...
def replace_leaf(t, find_value, replace_value):
    ...
    if is_leaf(t):
        if s == find_value:
            s = replace_value
        return tree(s)
    return tree(s, [replace_leaf(b, find_value, replace_value) for b in branches(t)])
...
def quadratic(x, a, b, c):
    ...
    "*** YOUR CODE HERE ***"
    d = -b/(2*a)
    y1 = a*upper_bound(x)*upper_bound(x)+b*upper_bound(x)+c
    y2 = a*lower_bound(x)*lower_bound(x)+b*lower_bound(x)+c
    if d < upper_bound(x) and d > lower_bound(x):
        m = a*d*d+b*d+c
        return interval(min(y1,y2,m),max(y1,y2,m))
