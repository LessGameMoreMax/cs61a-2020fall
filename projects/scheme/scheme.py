...
def eval_all(expressions, env):
    ...
    temp = expressions
    while temp != nil:
        result = scheme_eval(temp.first, env, True)
        temp = temp.rest
    return result
    # END PROBLEM 7
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        temp = self
        while temp != None:
            result = temp.bindings.get(symbol)
            if result != None:
                return result
            temp = temp.parent
        # END PROBLEM 2
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(target.first, LambdaProcedure(formals, expressions.rest, env))
        return target.first
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    # v = True
    # while temp != nil:
    #     v = scheme_eval(temp.first, env, True)
    #     if is_false_primitive(v):
    #         return v
    #     temp = temp.rest
    # return v
    ...
    if expressions is nil:
        ...
        if is_false_primitive(first_expr): 
            return False
        elif is_true_primitive(first_expr):
...
def make_let_frame(bindings, env):
    ...
    temp = bindings
    while temp != nil:
        validate_form(temp.first, 2, 2)
        name = temp.first.first
        value = scheme_eval(temp.first.rest.first, env)
...
class Promise(object):
    ...
    def evaluate(self):
        if self.expression is not None:
            value = scheme_eval(self.expression, self.env, True)
