    ...
    for branch in branches(t):
        if is_tree(branch):
            if berry_finder(branch):
                return True
        if is_leaf(branch) and label(branch) == 'berry':
            return True
    return False
...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    # return [deck[0] if len(deck)%2==0 else deck[len(deck)//2]] + riffle(deck[1:] if len(deck)%2==0 else deck[:len(deck)//2] + deck[len(deck)//2+1:])
...
def add_trees(t1, t2):
    ...
    m = min(len(b1), len(b2))
    for i in range(m):
        result.append(add_trees(b1[i], b2[i]))
    for i in range(m, len(b1)):
        result.append(b1[i])
    for i in range(m, len(b2)):
        result.append(b2[i])
    return tree(label(t1)+label(t2), result)
