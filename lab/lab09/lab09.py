...
def subseqs(s):
    ...
    if len(s) == 0:
        ...
        r = subseqs(s[1:])
        return r[:] + insert_into_all(s[0], r)
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    while i != n//2+1:
        if n-i != i:
            r = r + num_trees(n-i) * num_trees(i) * 2
        else:
            r = r + num_trees(n-i) * num_trees(i)
        i += 1
    return r
...
class Keyboard:
    ...
    def typing(self, typing_input):
        ...
        for p in typing_input:
            result += self.buttons[p].key
            self.buttons[p].times_pressed += 1
        return result
